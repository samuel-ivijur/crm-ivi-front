...
import { ModalImportFormStepper } from "./modal-import-form-stepper";
    ...
    setExpectedColumnsToRows((prev) => {
      ...
      Object.keys(newValue).forEach((key) => {
        if (newValue[key] === row && value !== key) {
          toast({
            title: 'Essa coluna já foi selecionada',
            description: 'Essa coluna já foi selecionada',
          });
          newValue[key] = '';
        }
      });
          ...
          <div className="mt-6">
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {docExampleUrl && (
                    ...
                    >
                      <CloudDownload size={16} /> Baixar exemplo de planilha
                    </Button>
                  )}
                </div>
                    ...
                    {(Array.isArray(rows) && rows.length > 0) &&
                      Object.keys(rows[0]).map((row) => (
                        <div key={row} className="validate-column-box">
                          ...
                          <div className="mt-2" />
                          <span style={{ color: 'rgba(0,0,0,.5)' }}>{rows[0][row]}</span>
                          <div className="mt-2" />
                          <span style={{ color: 'rgba(0,0,0,.2)' }}>
                            {rows.length > 1 && rows[1][row]}
                          </span>
                        </div>
                      ))}
                  </div>
                    ...
                    {rows.map((row) => (
                      <TableRow key={row[expectedColumns[0].key]}>
                        {expectedColumns.map((column) => {
                          ...
                          return <TableCell key={column.key}>{columnValue}</TableCell>
                        })}
                      </TableRow>
                    ))}
                  </TableBody>
            ...
            <div className="flex justify-between mt-6">
              ...
              <Button
                ...
                onClick={async () => {
                  setLoading(true);
                  await finish(rows, expectedColumnsToRows);
                  setLoading(false);
                }}
